...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\t KUPINANG KAMU DGAN  : [green] BISMILLAHIRRAHMANIRRAHIM [purple] '))
		...
		print(f'  {u}[{h}•{u}]{h} LOGIN BERHASIL.........Jalankan Lagi Perintahnya 👉kribo-king.py!!!!{k} ');time.sleep(1)
		exit()
	except Exception as e:
...
def menu(my_name,my_id):
	...
	cetak(nel('\tSelamat Datang [green]%s[purple] HECKEL 🤣🤣'%(my_name)))
	alvino_xy(f'{u}ID  : '+str(my_id))
	alvino_xy(f'{h}IP  : {ip}')
	cetak(nel('\t[bold cyan]           • PILIH MENU CRACK • [/bold cyan]'))
	print('')
...
def setting():
	...
	if hu in ['1','01']:
		...
		exit()
	cetak(nel('\t[bold cyan]             • PILIH METODE LOGIN• [/bold cyan]'))
	print('') 
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{b}[KRIBO]{P}[{k}{loop}{P}/{h}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
